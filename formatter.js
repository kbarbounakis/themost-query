...
class SqlFormatter {
        ...
        if (typeof comparison === 'object') {
            ...
            if (compares.length === 0)
                ...
                for (var i = 0; i < compares.length; i++) {
                    ...
                    switch (key) {
                        case '$eq':
                            arr.push('(%s'.concat('=', escapedValue, ')'));
                            break;
                        case '$lt':
                            arr.push('(%s'.concat('<', escapedValue, ')'));
                            break;
                        ...
                        case '$gte':
                            ...
                            break;
                        case '$ne':
                            arr.push('(NOT %s'.concat('=', escapedValue, ')'));
                            break;
                        case '$in':
                            arr.push('(%s'.concat('(', escapedValue, '))'));
                            break;
    ...
    isComparison(obj) {
        var key = getOwnPropertyName(obj);
    ...
    formatSelect(obj) {
        ...
        //get entity name
        var entity = getOwnPropertyName(obj.$select);
        var joins = [];
        ...
        if (obj.$expand !== null) {
            ...
            _.forEach(joins, function (x) {
                ...
                if (_.isArray(x.$with)) {
                    ...
                    var left = x.$with[0], right = x.$with[1],
    ...
    formatField(obj) {
        ...
        if (typeof obj === 'object') {
            ...
            //get table name
            var tableName = getOwnPropertyName(obj);
            var fields = [];
    ...
    formatInsert(obj) {
        ...
        //get entity name
        var entity = getOwnPropertyName(obj.$insert);
        //get entity fields
        ...
        for (var prop in obj1)
            if (hasOwnProperty(obj1, prop))
                props.push(prop);
        sql = sql.concat('INSERT INTO ', self.escapeName(entity), '(', _.map(props, function (x) { return self.escapeName(x); }).join(', '), ') VALUES (', _.map(props, function (x) {
            var value = obj1[x];
    ...
    formatUpdate(obj) {
        ...
        //get entity name
        var entity = getOwnPropertyName(obj.$update);
        //get entity fields
    ...
    formatFieldEx(obj, format) {
        ...
        //get property
        var prop = getOwnPropertyName(obj);
        if (_.isNil(prop))
            return null;
    ...
    $substr(p0, pos, length) {
        return this.$substring(0, pos, length);
    }
